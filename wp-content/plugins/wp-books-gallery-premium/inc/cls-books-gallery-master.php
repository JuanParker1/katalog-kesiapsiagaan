...
class WBG_Master
{
	...
	private function wbg_trigger_admin_hooks() {
		...
		$this->wbg_loader->add_action('init', $wbg_admin, WBG_PRFX . 'custom_post_type', 10, 1);
		$this->wbg_loader->add_action('init', $wbg_admin, WBG_PRFX . 'taxonomy_for_books', 10, 1);
		$this->wbg_loader->add_action('admin_menu', $wbg_admin, WBG_PRFX . 'admin_menu', 12, 1);
		...
		$this->wbg_loader->add_action('add_meta_boxes', $wbg_admin, WBG_PRFX . 'book_details_metaboxes', 10, 2);
		$this->wbg_loader->add_action('save_post', $wbg_admin, WBG_PRFX . 'save_book_meta', 1, 1);
		
		if ( wbg_fs()->is_plan__premium_only('pro') ) {
			// Search order ajax load
			$this->wbg_loader->add_action('wp_ajax_search_item_order', $wbg_admin, 'wbgp_set_search_item_order' );
			$this->wbg_loader->add_action('wp_ajax_nopriv_search_item_order', $wbg_admin, 'wbgp_set_search_item_order' );
		}
	}
